...
class SupabaseConfig(BaseSettings):
    ...
    anon_key: SecretStr = Field(default="", json_schema_extra={"env": "SUPABASE_ANON_KEY"})
    service_role_key: SecretStr = Field(default="", json_schema_extra={"env": "SUPABASE_SERVICE_ROLE_KEY"})
    
    # Database connection for direct access
    db_host: str = Field(default="", json_schema_extra={"env": "SUPABASE_DB_HOST"})
    db_port: int = Field(default=5432, json_schema_extra={"env": "SUPABASE_DB_PORT"})
    db_name: str = Field(default="postgres", json_schema_extra={"env": "SUPABASE_DB_NAME"})
    db_user: str = Field(default="postgres", json_schema_extra={"env": "SUPABASE_DB_USER"})
    db_password: SecretStr = Field(default="", json_schema_extra={"env": "SUPABASE_DB_PASSWORD"})
...
class Settings(BaseSettings):
    ...
    def SUPABASE_HOST(self) -> str:
        """Get Supabase database host"""
        return self.supabase.db_host
    
    @property
    def SUPABASE_PORT(self) -> int:
        """Get Supabase database port"""
        return self.supabase.db_port
    
    @property
    def SUPABASE_DB_NAME(self) -> str:
        """Get Supabase database name"""
        return self.supabase.db_name
    
    @property
    def SUPABASE_USER(self) -> str:
        """Get Supabase database user"""
        return self.supabase.db_user
    
    @property
    def SUPABASE_PASSWORD(self) -> str:
        """Get Supabase database password"""
        return self.supabase.db_password.get_secret_value()

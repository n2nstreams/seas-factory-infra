...
# Night 56: Additional subscription-related functions for admin routes compatibility

class AccessLevel(Enum):
    """Access levels for subscription tiers"""
    STARTER = "starter"
    PRO = "pro"
    GROWTH = "growth"
...
class TenantSubscription:
    ...
    tenant_id: str
    subscription_level: AccessLevel
    active: bool = True
...
class SubscriptionVerifier:
    """Verifies tenant subscriptions and access levels"""
    
    def __init__(self):
        # Mock subscription data for demo purposes
        self._subscriptions = {}
    
    async def get_tenant_subscription(self, tenant_id: str) -> Optional[TenantSubscription]:
        """Get subscription for a tenant"""
        # For demo purposes, return a PRO subscription for any tenant
        return TenantSubscription(
            tenant_id=tenant_id,
            subscription_level=AccessLevel.PRO,
            active=True
        )
    
    def check_access_level(self, subscription: Optional[TenantSubscription], required_level: AccessLevel) -> bool:
        """Check if subscription meets required access level"""
        ...
        level_hierarchy = {
            ...
            AccessLevel.PRO: 2,
            AccessLevel.GROWTH: 3
        }
        
        return level_hierarchy.get(subscription.subscription_level, 0) >= level_hierarchy.get(required_level, 0)

# Global subscription verifier instance
subscription_verifier = SubscriptionVerifier()

async def get_tenant_subscription(tenant_id: str) -> Optional[TenantSubscription]:
    """Get tenant subscription information"""
    return await subscription_verifier.get_tenant_subscription(tenant_id)

def require_subscription(access_level: AccessLevel):

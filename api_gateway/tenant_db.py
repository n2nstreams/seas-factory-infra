...
class TenantDatabase:
    ...
    def __init__(self):
        ...
        if database_url:
            ...
            self.db_name = parsed.path.lstrip('/') or "saas_factory"
            ...
            self.db_name = os.getenv("DB_NAME", "saas_factory")
    ...
    async def get_tenant_connection(self, tenant_context: TenantContext):
        ...
        async with self._pool.acquire() as conn:
            try:
                ...
                await conn.execute(
                    f"SET app.current_tenant_id = '{tenant_context.tenant_id}'"
                )
                
                if tenant_context.user_id:
                    await conn.execute(
                        f"SET app.current_user_id = '{tenant_context.user_id}'"
                    )
                
                if tenant_context.user_role:
                    await conn.execute(
                        f"SET app.current_user_role = '{tenant_context.user_role}'"
                    )
